# AI Bookmark Organizer：MVP 技术架构方案
...
2. 扫描选择：书签树、文件夹选择、书签选择和数量统计。
3. 方案审核：展示最终生成的完整目录树，并在目录节点下展示将要移入的书签；支持逐条修改和排除。
4. 执行结果：成功、失败、重试和撤销最近一次整理。

模型请求由 Dashboard 发起。每完成一个批次就持久化结果；页面意外关闭时只丢失当前未完成请求，重新打开后从最后一个完整批次继续。
...
为兼容不同上下文长度和限流策略的自定义模型，使用两阶段、可恢复的分批流程：
...
首版顺序请求，不并发轰炸用户 API。遇到 `429` 或 `5xx` 时读取 `Retry-After`，否则指数退避，最多自动重试两次。
